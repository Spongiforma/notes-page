...
Let X be a topological space. Suppose that \\(\mathcal{C}\\) is a collection of open sets of X such that for each open set U of and each x in U, there is an element \\(C\\) of \\(\mathcal{C}\\) such that \\(x \in C \subset U\\). Then \\(\mathcal{C}\\) is a basis for the topology of X.